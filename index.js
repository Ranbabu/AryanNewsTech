/* Workers AI output से base64 निकालने का robust helper */
...
export default {
  async fetch(request, env) {
    ...
    // 📋 CATALOG ROUTE (CF_API_TOKEN + CF_ACCOUNT_ID secrets हों तो)
    if (url.pathname === "/cf/models") {
      const tok = env.CF_API_TOKEN, acc = env.CF_ACCOUNT_ID;
      if (!tok || !acc) {
        return new Response(JSON.stringify({ error: "CF_API_TOKEN और CF_ACCOUNT_ID secrets सेट नहीं हैं। आसान तरीका: Dashboard → Workers AI → Models में IDs देखें और variable CF_IMAGE_MODELS में डालें" }), {
      ...
      try {
        ...
        const list = (d.result || [])
          .filter(m => ((m.task && m.task.name) || "") === "Text to Image" || /image|flux|diffusion|dreamshaper|lucid|pruna|qwen|grok/i.test(m.name || ""))
          .map(m => m.name);
    ...
    // ============================================================
    // ☁️ WORKERS AI IMAGE — आपके account वाली सही IDs सबसे पहले
    ...
    if (url.pathname === "/cf/image" && request.method === "POST") {
      ...
      /* variable CF_IMAGE_MODELS में अपनी IDs डालें तो वही पहले चलेंगी */
      ...
      /* हर ID को उसकी दोनों संभावित forms (@cf/ सहित/रहित) में आज़माएँ */
      const candidates = [];
      ...
      for (const m of candidates) {
        try {
          const out = await env.AI.run(m, { prompt: shortPrompt, width: 1024, height: 576 });
          ...
          /* model चला पर image खाली → default size से एक बार और */
          try {
            const out2 = await env.AI.run(m, { prompt: shortPrompt });
            const b642 = extractB64(out2);
            if (b642) {
              return new Response(JSON.stringify({ b64: b642, mimeType: "image/png" }), {
                status: 200, headers: { "Content-Type": "application/json", ...corsHeaders }
              });
            }
            errs.push(m + "(default): keys=[" + Object.keys(out2 || {}).join(",") + "]");
          } catch (e2) {
            errs.push(m + "(default): " + (e2 && e2.message ? e2.message : String(e2)));
          }
        } catch (e) {
          errs.push(m + ": " + (e && e.message ? e.message : String(e)));
        }
      }
      return new Response(JSON.stringify({ error: "Workers AI सभी मॉडल fail: " + errs.join(" | ") + " || Dashboard → Workers AI → Models/Playground से सही ID लेकर variable CF_IMAGE_MODELS में डालें" }), {
        status: 502, headers: { "Content-Type": "application/json", ...corsHeaders }
      });
    }
